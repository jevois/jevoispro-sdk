...
#define SENSOR_WRITE16(reg, val)                                        \
  acamera_sbus_write_u8(&p_ctx->sbus, reg, ((val) >> 8) & 0xff);        \
  acamera_sbus_write_u8(&p_ctx->sbus, reg + 1, (val) & 0xff);

#define SENSOR_WRITE24(reg, val)                                        \
  acamera_sbus_write_u8(&p_ctx->sbus, reg, ((val) >> 16) & 0xff);       \
  acamera_sbus_write_u8(&p_ctx->sbus, reg + 1, ((val) >> 8) & 0xff);    \
  acamera_sbus_write_u8(&p_ctx->sbus, reg + 2, (val) & 0xff);
...
#define SENSOR_READ24(reg)                                              \
  ...
  ret |= acamera_sbus_read_u8(&p_ctx->sbus, reg) << 16;                 \
...
static const char p_sensor_data[] = SENSOR__AR0234_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR__AR0234_ISP_SEQUENCE_DEFAULT;
#else
...
static void sensor_update( void *ctx )
{
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt ) {
      // ---------- Start Changes -------------
      SENSOR_WRITE8(0x3022, 0x01);
      
      // ---------- Analog Gain -------------
      if ( p_ctx->gain_cnt ) {
        SENSOR_WRITE16(0x3060, p_ctx->again[p_ctx->again_delay]);
        p_ctx->gain_cnt--;
      }
      ...
      if ( p_ctx->int_cnt )
      {
        ...
        case WDR_MODE_LINEAR:
          SENSOR_WRITE16(0x3012,  p_ctx->int_time_S);
          break;
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    case WDR_MODE_LINEAR:
      pr_info("JEVOIS: loading sensor register sequence %d\n", setting_num);
    ...
    p_ctx->s_fps = param->modes_table[mode].fps / 256;
    SENSOR_READ16(0x300a);
    p_ctx->vmax = ret;
    p_ctx->max_S = ret;
    ...
    param->total.width = param->active.width + 12;
...
void sensor_deinit_ar0234( void *ctx )
{
...
//--------------------Initialization------------------------------------------------------------
void sensor_init_ar0234( void **ctx, sensor_control_t *ctrl, void *sbp )
{
  ...
  system_timer_usleep(50000); // reset delay
    ...
    *ctx = &s_ctx;
    ...
    s_ctx.param.isp_context_seq.seq_num = SENSOR_AR0234_ISP_CONTEXT_SEQ;
  ...
  // Load initial sequence, in this case, looks like 1920x1200@30 is a good init
  sensor_set_mode(&s_ctx, 3);
