...
int clk_am_enable(sensor_bringup_t* sensor_bp, const char* propname)
{
	...
	np = sensor_bp->np;
    ret = of_property_read_u32(np, "mclk-freq", &freq);
    if (ret) {
      pr_info("JEVOIS: no value for mclk-freq in DT. Default sensor clock 24MHz\n");
      freq = 24000000;
    }
    
	//clk_set_rate(clk, 24000000);
    pr_info("JEVOIS: setting camera sensor clock to %d kHz\n", freq / 1000);
	clk_set_rate(clk, freq);//JEVOIS 
	clk_prepare_enable(clk);
	clk_val = clk_get_rate(clk);
	pr_info("JEVOIS: sensor init clock is %d kHZ\n",clk_val/1000);
