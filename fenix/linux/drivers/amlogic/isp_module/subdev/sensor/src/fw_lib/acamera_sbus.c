...
uint8_t acamera_sbus_read_u8( acamera_sbus_t *p_bus, uintptr_t addr )
{
  uint8_t ret;
    ...
    if ( p_bus->mask & SBUS_MASK_SAMPLE_8BITS ) {
        ret = ( uint8_t )( p_bus->read_sample( p_bus, sbus_update_address( p_bus, addr ), 1 ) );
        //printk("JEVOIS: acamera_sbus_read_u8(subdev/sensor/src/fwlib) reg=%x, val=%x\n", (uint16_t)addr, ret);
        return ret;
    } else {
        // error here
        return 0xff;
...
{
  //printk("JEVOIS: acamera_sbus_write_u8(subdev/sensor/src/fwlib) reg=%x val=%x\n", (uint16_t)addr, sample);
    if ( p_bus->mask & SBUS_MASK_SAMPLE_8BITS ) {
