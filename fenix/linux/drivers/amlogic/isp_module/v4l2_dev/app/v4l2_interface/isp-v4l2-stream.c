...
int isp_v4l2_stream_try_format( isp_v4l2_stream_t *pstream, struct v4l2_format *f )
{
    isp_v4l2_fmt_t *tfmt;
    int i;
    LOG( LOG_ERR, "[Stream#%d] try fmt type: %u, pixelformat: 0x%x, width: %u, height: %u.\n",
    ...
    if (f->fmt.pix_mp.pixelformat == ISP_V4L2_PIX_FMT_META) {
        ...
        if ( !tfmt ) {
            LOG( LOG_CRIT, "[Stream#%d] format 0x%08x is not supported, setting default format 0x%08x.\n",
                 pstream->stream_id, f->fmt.pix_mp.pixelformat, ISP_DEFAULT_FORMAT );
    ...
    if ( tfmt->fourcc == V4L2_PIX_FMT_SBGGR16 ) {
        ...
        if ( fw_intf_is_isp_started() ) {
            ...
            LOG( LOG_ERR, "[Stream#%d] Changing the number of planes according preset %d to exposures %d=>%d.\n", pstream->stream_id, spreset, tfmt->planes, exposures_preset );
...
#endif
    {
        if ( f->fmt.pix_mp.width == 0 || f->fmt.pix_mp.height == 0 ) {
            if ( fw_intf_is_isp_started() ) {
                ...
                f->fmt.pix_mp.width = width_cur;
                f->fmt.pix_mp.height = height_cur;
            } else {
                f->fmt.pix_mp.width = 1920;
                f->fmt.pix_mp.height = 1080;
            }
        }
        v4l_bound_align_image( &f->fmt.pix_mp.width, 48, ISP_V4L2_MAX_WIDTH, 2,
                               &f->fmt.pix_mp.height, 32, ISP_V4L2_MAX_HEIGHT, 0, 0 );
    }
...
static int isp_v4l2_get_crop_type(isp_v4l2_stream_t *pstream)
{
    ...
    case V4L2_STREAM_TYPE_DS1:
      //s_type = CROP_DS; ////JEVOIS
      s_type = SCALER;////JEVOIS
    break;
    default:
                            ...
                            const struct v4l2_crop *crop)
{
    ...
    if (pstream->stream_type == V4L2_STREAM_TYPE_FR) {
        ...
        acamera_command( TAML_SCALER, SCALER_SRC_HEIGHT,
                            height, COMMAND_SET, &ret_val );
        LOG(LOG_ERR, "JEVOIS: setting scaler src to %dx%d", width, height);
    }
