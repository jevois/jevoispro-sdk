...
struct amlogic_pcie {
	...
	u32			rst_mod;
	u32			pwr_ctl;
    int power1_gpio; // JEVOIS
    int power2_gpio; // JEVOIS
	int	reset2_gpio; // JEVOIS
};
...
static void amlogic_pcie_assert_reset(struct amlogic_pcie *amlogic_pcie)
{
	...
	if (amlogic_pcie->gpio_type == 0) {
		...
		if (gpio_is_valid(amlogic_pcie->reset_gpio)) {
            ...
            // JEVOIS: reset2
            if (amlogic_pcie->reset2_gpio >= 0)
              devm_gpio_request(dev, amlogic_pcie->reset2_gpio, "RESET2");
            gpio_direction_output(amlogic_pcie->reset2_gpio, 0);
            if (gpio_is_valid(amlogic_pcie->reset2_gpio)) {
              dev_info(amlogic_pcie->pp.dev, "GPIO normal: amlogic_pcie_assert_reset2\n");
              gpio_set_value_cansleep(amlogic_pcie->reset2_gpio, 0);
              usleep_range(5000, 6000);
              gpio_set_value_cansleep(amlogic_pcie->reset2_gpio, 1);
            }
        }
    }   
}
...
#define PCIE_PLL_RATE 100000000
    ...
    // JEVOIS: second reset
	amlogic_pcie->reset2_gpio = of_get_named_gpio(np, "reset2-gpio", 0);
    ...
    // JEVOIS: turn on power1
	amlogic_pcie->power1_gpio = of_get_named_gpio(np, "power1-gpio", 0);
    if (amlogic_pcie->power1_gpio >= 0)
    {
      // JEVOIS: reset should be low during power up
      devm_gpio_request(dev, amlogic_pcie->reset_gpio, "RESET");
      gpio_direction_output(amlogic_pcie->reset_gpio, 0);
      if (gpio_is_valid(amlogic_pcie->reset_gpio)) {
        dev_info(amlogic_pcie->pp.dev, "JEVOIS: Assert RESET gpio\n");
        gpio_set_value_cansleep(amlogic_pcie->reset_gpio, 0);
      }

      // JEVOIS: now turn on power1:
      devm_gpio_request(dev, amlogic_pcie->power1_gpio, "POWER1");
      gpio_direction_output(amlogic_pcie->power1_gpio, 0);
      if (gpio_is_valid(amlogic_pcie->power1_gpio)) {
        dev_info(amlogic_pcie->pp.dev, "JEVOIS: Assert POWER1 gpio\n");
        gpio_set_value_cansleep(amlogic_pcie->power1_gpio, 1);
      }
      // JEVOIS: we hold reset until later (amlogic_pcie_assert_reset)
    }
    else dev_info(amlogic_pcie->pp.dev, "JEVOIS: no valid power1-gpio in device tree\n");

    // JEVOIS: turn on power2
	amlogic_pcie->power2_gpio = of_get_named_gpio(np, "power2-gpio", 0);
    if (amlogic_pcie->power2_gpio >= 0)
    {
      // JEVOIS: reset2 should be low during power up
      devm_gpio_request(dev, amlogic_pcie->reset2_gpio, "RESET2");
      gpio_direction_output(amlogic_pcie->reset2_gpio, 0);
      if (gpio_is_valid(amlogic_pcie->reset2_gpio)) {
        dev_info(amlogic_pcie->pp.dev, "JEVOIS: Assert RESET2 gpio\n");
        gpio_set_value_cansleep(amlogic_pcie->reset2_gpio, 0);
      }

      // JEVOIS: now turn on power2:
      devm_gpio_request(dev, amlogic_pcie->power2_gpio, "POWER2");
      gpio_direction_output(amlogic_pcie->power2_gpio, 0);
      if (gpio_is_valid(amlogic_pcie->power2_gpio)) {
        dev_info(amlogic_pcie->pp.dev, "JEVOIS: Assert POWER2 gpio\n");
        gpio_set_value_cansleep(amlogic_pcie->power2_gpio, 1);
      }
      // JEVOIS: we hold reset2 until later (amlogic_pcie_assert_reset)
    }
    else dev_info(amlogic_pcie->pp.dev, "JEVOIS: no valid power2-gpio in device tree\n");

    //msleep(10); // JEVOIS: TPUs need at least 10ms to boot up
