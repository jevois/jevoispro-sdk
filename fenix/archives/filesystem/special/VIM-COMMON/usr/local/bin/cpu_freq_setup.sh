...
elif [ $BOARD == "VIM3" -o $BOARD == "JVPRO" ]; then
    FREQ_VALUE_Little=($(cat /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq))
...
if [ $BOARD == "VIM3" -o $BOARD == "JVPRO" ]; then
    m=0
    while(( m<${num_big} ))
    do
...
if [[ ${BOARD} == "VIM3L" && ${selected_Little_core} == ${FREQ_VALUE_Little} ]]; then
    exit
elif [[ ( ${BOARD} == "VIM3" -o $BOARD == "JVPRO" ) && ${selected_Big_core} == ${FREQ_VALUE_Big} && ${selected_Little_core} == ${FREQ_VALUE_Little} ]]; then
    exit
fi
